...
class Senescence_grid():
    ...
    def cumulative_func(self, r):
        """cumulative probability distribution for the probability of binding a ligand at distance r, accounting for the
        autocrine binding of ligands
        
        Parameters
        ----------
        r: float
            distance from cell which emits ligand [m]
            ...
            if len(self.Tstruct) == 0:
                ...
                while additional_time_to_event > T1:
                    ...
                    if cell_type == 1:
                        ...
                        for co in coords:
                            # include relayed_reaction_time, as want to specify the time after initial_time when this
                            # cell will become senescent
                            self.update_juxt_cells_delay([(co[0], co[1])], self.delay + delayed_reaction_time)
                            if verbose:
                                print("Added juxt to Tstruct")
